...
def draw_ship_menu_view(screen, app_state, fonts, config_module):
    """
    Draw the ship selection submenu view.
    ...
    if menu_items:
        ...
        for i, item in enumerate(menu_items): # item is now a MenuItem object
            ...
            try:
                # item.color_key should be a string like "SIDEBAR_SCHEMATICS"
    ...
    # Display ship selection content
    ...
    instruction_text = "Select a ship to view its 3D model"
    ...
    # Display selected ship info if available
    if menu_items and 0 <= selected_index < len(menu_items):
        selected_ship = menu_items[selected_index]
        if selected_ship.data and 'ship_model' in selected_ship.data:
            ship_info_text = f"Ship Model: {selected_ship.data['ship_model']}"
            ship_info_surface = instruction_font.render(ship_info_text, True, config_module.Theme.FOREGROUND)
            ship_info_rect = ship_info_surface.get_rect(centerx=main_content_rect.centerx, y=instruction_rect.bottom + 30)
            screen.blit(ship_info_surface, ship_info_rect)
    ...
    back_rect = back_surface.get_rect(centerx=main_content_rect.centerx, y=main_content_rect.bottom - 60)
